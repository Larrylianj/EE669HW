...
void encode_one_macroblock_high (Macroblock *currMB)
{
  ...
  Info8x8     best;
    ...
    int skip_all_intra_mode = 0; // if  it = 1, skip all intra modes, else do intra modes
    int total_block_number = p_Vid->p_Inp->source.width[0]/16 * p_Vid->p_Inp->source.height[0]/16;
    int me_choice = 0; // if me_choice = 1, do ME only for 16x16, 16x8, 8x16 blocks, else do ME for all blocks   ************************************************************************************************************/
    /************************************************************************************************************/
  ...
  if (!intra)
  {
    ...
    if (p_Inp->CtxAdptLagrangeMult == 1)
    {
      ...
      current_block_SAD = p_Vid->p_ffast_me->BlockSAD[0][0][1][0][0];
...
//      printf("num of macroblocks in a silce = %d\n",currSlice->num_mb);
...
//      printf("frame_total SAD = %lld\n",frame_total_SAD);
      ...
      if (block_total_number != 0) {
          if (frame_total_SAD/block_total_number >= w_value* current_block_SAD) {
              me_choice = 1;
          }else{
              me_choice = 0;
          }
